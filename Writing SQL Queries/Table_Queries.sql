...
-- Question 1: Get a list of all film titles in alphabetical order
SELECT * FROM film ORDER BY title ASC;

-- Question 2: Find the descripton, release year, length, and ratng for the movie “KENTUCKIAN GIANT”
SELECT description, release_year, length, rating 
FROM film 
WHERE title = 'KENTUCKIAN GIANT';

-- Question 3: Find the first name and last name of each employee (staff table). 
-- Your query should include the last name first, and then the first name.
SELECT last_name AS "Last Name", first_name AS "First Name"
FROM staff;

-- Question 4: Repeat the query above, but this time, the results should include 
-- only one column with the format last name, first name. The output column should be named “name”

SELECT CONCAT(last_name, ", ", first_name) as "name"
FROM staff;

-- Question 5: Get the number of customers. The output should be a single number. Name the column
-- “num_customers”

SELECT COUNT(*) as "num_customers"
FROM customer;

-- Question 6: Get the number of customers who are active vs inactive in the system.
SELECT 
    CASE 
...
-- Question 7: Get the average amount a customer spends on a rental
SELECT AVG(amount)
FROM payment;

-- Question 8: Get maximum amount any customer has spent on a rental.
SELECT MAX(amount)
FROM payment;

-- Question 9: Get a list of the actors. 
-- The results should include only one column with the format last
-- name, first name. The column should be named “actor_name” The results should be
-- sorted be sorted alphabetically by the last name (ascending).

SELECT CONCAT(last_name,", ",first_name) as "actor_name"
FROM actor
ORDER BY actor_name ASC;

-- Question 10: Repeat this query above, but the results should be in reverse order

SELECT CONCAT(last_name,", ",first_name) as "actor_name"
FROM actor
ORDER BY actor_name DESC;

-- Question 11:Repeat the query again, this time get only actors whose last names start with ‘M’ or ‘V’. 
...
-- Question 12:Repeat the query again, this time get only actors whose last names start with leters between
-- ‘M’ and ‘V’ inclusive. Order the results alphabetically by last name (ascending).

SELECT CONCAT(last_name, ", ", first_name) AS "actor_name"
FROM actor
WHERE last_name BETWEEN 'M' AND 'V'
ORDER BY last_name ASC;

-- Question 13: Get a list of each customer ID and the number of rentals they have in their history. 
-- Name this column Number of Rentals.

SELECT customer_id, COUNT(*) AS "Number of Rentals"
FROM rental
